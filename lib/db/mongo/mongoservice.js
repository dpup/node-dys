...
MongoService.prototype.count = function(col, query, callback) {
  callback = callback ? base.wrap(this.ctx, callback) : DEFAULT_CB;
  this.find(col, query, {}, function(err, cursor) {
...
MongoService.prototype.ensureIndex = function(col, fields, callback) {
  callback = callback ? base.wrap(this.ctx, callback) : DEFAULT_CB;
  this.db.collection(col, function(err, collection) {
    if (err) return callback(err, null);
		collection.ensureIndex(fields, callback);
  }); 
};
