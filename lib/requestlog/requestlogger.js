...
RequestLogger.prototype.setLogDir = function(logDir) {
  ...
  this.logger_.info('Logging requests to ' + this.currentFile_);
};
...
RequestLogger.prototype.flushNow = function() {
  ...
  fs.open(fileName, 'a+', '0666', function(err, fd) {
    if (err) {
      logger.warn('Failed to open log file [' + fileName + '] ' + err.stack);
    } else {
      fs.write(fd, buffer, 0, buffer.length, null, function(err, written) {
        if (err) {
          logger.warn('Failed to write to log file [' + fileName + '] ' + err.stack);
        } else {
          logger.fine(written + ' bytes written to ' + fileName);
          fs.close(fd);
        }
      });
    }
...
/**
 * Intercepts the request and sets up logg.
...
RequestLogger.prototype.execute = function(ctx, proceed) {
  if (!this.logDir_) {
    this.logger_.warn('No log directory set');
    proceed();
    return;
  }
...
RequestLogger.prototype.maybeRotateLogFile_ = function() {
  ...
  if (this.currentDate_ != nowDate) {
    ...
    this.currentFile_ = this.getLogName_();
    this.logger_.info('Now logging requests to ' + this.currentFile_);
  }
};
