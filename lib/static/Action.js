 ...
 * @constructor
 */
function Action(ctx, basePath, fileReader, options) {
   ...
   * @private
   */
  this.mimes_ = options.mimes;
  
  /**
   * How long the browser should cache files for, in milliseconds.
   * @type {number}
   * @private
   */
  this.cacheLifetime_ = options.cacheLifetime;
...
Action.prototype.execute = function(ctx) {
  ...
  this.fileReader_.stat(path, base.wrap(ctx, function(err, stats) {
    if (err) {
...
Action.prototype.writeResponse_ = function(ctx, stats, path) {  
  ...
  if (ifNoneMatch && ifNoneMatch.indexOf(etag) != -1 ||
    ...
    if (req.method == 'HEAD') {
      ...
      this.fileReader_.pipeFile(path, fsOptions, res, base.wrap(ctx, function(err) {
        if (err) throw err;
        this.filesServed_++;
