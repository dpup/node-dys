...
Server.prototype.registerProviders_ = function() { 
  ...
  context.registerProvider(
      'cookies', Server.provideCookies_, Context.Scope.REQUEST);

  context.registerProvider(
      'action-annotations',
      Server.provideActionAnnotations_, 
...
/**
 * Decodes the cookie header from a request and returns the object literal,
 * when registered as a request-scoped provider this allows decoding to be done
 * once per request.
 * @param {!Context} context
 * @return {Object}
 * @private
 */
Server.provideCookies_ = function(context) {
  var req = context.get('request');
  // TODO : Make cookies an object that exposed set()
  var cookies = {};
...
Server.provideActionAnnotations_ = function(context) {
  var action = context.get('action');
