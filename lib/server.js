...
/**
 * Starts up the server, returns command line args sync, though not all services
 * may be ready at startup.
 * @param {function()} callback Function executed when the server is ready.
 * @return {!Array.<string>} Non-flag command line arguments.
 */
Server.prototype.start = function(opt_callback) {
...
Server.prototype.initModules_ = function(opt_callback) {
  this.executeAsync_(
      this.modules_, 'init', this.startModules_.bind(this, opt_callback));
};


Server.prototype.startModules_ = function(opt_callback) {
  this.executeAsync_(
      this.modules_, 'start', this.onStart.bind(this, opt_callback));  
};


Server.prototype.executeAsync_ = function(arr, method, callback) {
